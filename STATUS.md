...
- Applicative, auxiliary, CAUS, and ITER/INTENS exponents remain UNSPECIFIED. The nonfinite exponents are established as EVENT `-n`, PARTICIPANT `-r`, and RESULT/PLACE `-t ~ -m`; participles and converbs are built compositionally from these rather than requiring separate exponents. Inalienable possessive-index exponents remain UNSPECIFIED. Nominal number is SG `Ø`, DU `-e`, PL `-u`, and COLL/PCL `-s` is established. The exact PART reflex in the planned `WHOLE-GEN + PART` construction and the exponent of its historically related verbal boundedness/aspect cognate remain UNSPECIFIED. Exact lexical distributions of root shapes and detailed active–stative verb-class assignments remain UNSPECIFIED.
...
- Historical derivations of the EVENT/PARTICIPANT/RESULT nominalizers, participial extension, converb case constructions, `r-` valency reducer, `s-` reciprocal, CAUS, and ITER/INTENS system remain to be developed.
...
Phonology, morphophonology, the high-level morphological architecture, productive case system, grammatical animacy, noun-class architecture, nominal number, and the nonfinite/derivational architecture are now canonical. The nonfinite module establishes EVENT `-n`, PARTICIPANT `-r`, RESULT/PLACE `-t ~ -m`, participles from PARTICIPANT `-r`, case-built converbs, GEN subjects for referential event nominals, subject-index suppression in nonfinites, and generalized /i/ linking for productive nonfinite derivatives. The verbal derivational module establishes pre-root `n-/r-/s-/Ø + k/t/p` participant series, productive VAL.RED and RECIP, a compact ITER/INTENS > CAUS order, DAT causees with retained original patients, and incompatibility of `r-/s-` with applicatives. Exact unresolved exponents remain listed above. The next decisive morphology work is exponent design for CAUS/ITER/INTENS/AUX/applicatives, inalienable possessive indexing, detailed active–stative lexical assignment, and the GEN-based partitive/verbal-boundedness cognate.
