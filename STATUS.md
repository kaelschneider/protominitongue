...
Phonology, morphophonology, nominal architecture, case, animacy, noun classes, number, nonfinite morphology, the core verbal exponent system, pronouns, and inalienable possession are now canonical and regression-tested. Verbal morphology now has affected-person APPL `-r`, means APPL `-m`, ITER/INTENS `-s`, CAUS `-t`, and a closed AUX slot whose members remain to be selected; multi-consonant repair proceeds from the rightmost problematic boundary. Pronouns distinguish PERSON vs THING only in independent 3P reference, preserve relic ERG `-ik`, GEN `-un`, DAT `-ar`, and old pronominal DU/PL morphology outside the fused 1P system. Inalienable possession uses person-only `-i/-a/-u` after nominal number and before case, while alienable possession remains GEN. The next decisive morphology work is selection of the AUX inventory/forms, detailed active–stative lexical assignment, and the GEN-based partitive construction.
