...
- Applicative, auxiliary, most derivational, nominalizer, participle, converb, and inalienable possessive-index exponents are UNSPECIFIED. Nominal number is now established as SG `Ø`, DU `-e`, PL `-u`, with pair-sensitive dual semantics and regular theme-conditioned surface fusion. COLL/PCL `-s` is established; the exact PART reflex in the planned `WHOLE-GEN + PART` construction and the exponent of its historically related verbal boundedness/aspect cognate remain UNSPECIFIED. Exact lexical distributions of root shapes and detailed active–stative verb-class assignments remain UNSPECIFIED.
...
Phonology, morphophonology, the high-level morphological architecture, productive case system, grammatical animacy, noun-class architecture, and nominal number are now canonical. The noun-class module establishes ANIMATE /i/, GROWING /a/, LAND/WATER /u/, MADE Ø/C, a relic SOCIAL stratum, class-conditioned LOC/PERL/INS pockets, person-sensitive DAT/COM, and a newer cross-cutting attributive classifier layer. The tested number module establishes SG `Ø`, DU `-e`, PL `-u`, pair-sensitive dual licensing, numeral-number interaction, tolerated phonological syncretism, and bounded collective/paucal `-s`; COLL/PCL-derived stems use generalized `-i-` rather than inherited class-theme case pockets. Exact unresolved forms remain listed above. The next decisive morphology tests are nominalization, derivation/AUX ordering, applicatives, and inalienable possessive indexing; the GEN-based partitive and verbal boundedness cognate should be developed with the relevant later modules.
